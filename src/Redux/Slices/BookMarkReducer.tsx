...
const BookMarkReducer = createSlice({
  ...
  reducers: {
    ...
    removeBookmMark: (state, action) => {
      state.bookmark = state.bookmark.filter((data: any) => data.id !== action.payload);
    },
    removeMyBookMark: (state, action) => {
      state.mybookmarkData = state.mybookmarkData.filter((data: any) => data.id !== action.payload);
    },
    ...
    updateBookMark: (state, action) => {
      ...
      state.bookmark = state.bookmark.map((item: any) => (item.id === action.payload.id ? bookmarkTemp : item));
    },
  }
}
);
